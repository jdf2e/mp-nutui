...
const {parseClassBinding, parseStyleBinding, parseEventParams} = require('./babel-plugins');
...
function parseTemplate(source) {
  ...
  return {
    wxmls: style_html(createMPtemplate(ast), 2),
    events
  }
  ...
  attrs.length && attrs.forEach(attr => {
    ...
    if(/^(@|v-on:)[^(v\-on)]/.test(attr)) {
      ...
      if(eventBar) {
        ...
        const {callName, dataStr} = parseEventParams(v);
        events.push(callName);
        attrStr += (` ${~eventBar.slice(1).indexOf('stop')? 'catch': 'bind'}:${evtName === 'click'? 'tap': evtName}="${callName}"`);
        attrStr += ` ${dataStr.join(' ')}`;
      }
...
// 小程序不支持$slots，需手动添加到data来抹平
// 小程序没有transition组件，需单独按小程序的做法实现一个transition组件
